...
const pasien = computed(() => page.props.auth.user?.pasien);

// Tab active state ('pasien' | 'sos')
const activeTab = ref('pasien');

// State untuk Pilihan Makanan yang ditambahkan sementara
const selectedFood = ref('Nasi Putih');
const foodPorsi = ref(1);
const foodList = ref([]);

// Daftar Referensi Makanan & Kalori
const masterFoods = [
...
// Fungsi Menambah Makanan ke List
const tambahMakanan = () => {
...
// Fungsi Hapus Makanan dari List
const hapusMakanan = (index) => {
    foodList.value.splice(index, 1);
};

// Computed Hitung Total Kalori
...
<template>
    <Head title="Dashboard Pasien - SIKEMBANG" />
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <!-- TAB 1: HALAMAN PASIEN & JURNAL -->
            <div v-if="activeTab === 'pasien'" class="space-y-6">
                <!-- FORM JURNAL KESEHATAN HARIAN -->
                <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-6">
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="bg-slate-50 p-5 rounded-2xl border border-slate-200/60 space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    v-model="journalForm.total_minum"
                                    type="number"
                                    class="w-full py-2 px-3 bg-white border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
                                />
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    v-model="journalForm.tekanan_darah"
                                    type="text"
                        ...
                        <div class="bg-slate-50 p-5 rounded-2xl border border-slate-200/60 space-y-4">
                            ...
                            <div>
                                ...
                                <select
                                    v-model="journalForm.kondisi_akses"
                                    class="w-full py-2 px-3 bg-white border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 focus:ring-2 focus:ring-emerald-500/20"
                                >
                        ...
                        <div class="bg-slate-50 p-5 rounded-2xl border border-slate-200/60 space-y-4">
                            ...
                            <div class="space-y-2">
                                <label class="block text-xs font-semibold text-slate-700">Pilih Menu (Khas Lamongan/Jatim)</label>
                                <select
                                    v-model="selectedFood"
                                    class="w-full py-2 px-3 bg-white border border-slate-200 rounded-xl text-xs font-semibold text-slate-800"
                                >
                                    ...
                                    <optgroup label="Menu Spesial Lamongan & Jatim">
                                        ...
                                        <option value="Tahu Tek / Tahu Telur">🧆 Tahu Tek / Tahu Telur (350 Kal)</option>
                                        <option value="Rawon Daging Tanpa Kuah Banyak">🥘 Rawon Daging Tanpa Kuah Banyak (300 Kal)</option>
                                        <option value="Rujak Cingur">🥗 Rujak Cingur (300 Kal)</option>
                                    </optgroup>
                                    <optgroup label="Lauk, Sayur & Jajanan">
                                        <option value="Botok Teri/Tahu">🥥 Botok Teri/Tahu (100 Kal)</option>
                                        <option value="Bandeng Presto (1 Potong)">🐟 Bandeng Presto - 1 Potong (200 Kal)</option>
                                        <option value="Sayur Asem (Kuah Sedikit)">🥣 Sayur Asem - Kuah Sedikit (50 Kal)</option>
                                        <option value="Wingko Babat (1 Potong Kecil)">🍘 Wingko Babat - 1 Potong Kecil (150 Kal)</option>
                                    </optgroup>
                                </select>
                                ...
                                <div class="flex items-center gap-2 pt-1">
                                    <input
                                        ...
                                        min="1"
                                        placeholder="Porsi"
                                        class="w-20 py-2 px-3 bg-white border border-slate-200 rounded-xl text-xs font-semibold text-slate-800"
                                    />
                                    <button
                                        type="button"
                                        @click="tambahMakanan"
                                        class="flex-1 py-2 px-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl shadow-sm transition-all"
                                    >
                                        + Tambah ke Jurnal
                                    </button>
                                </div>
                            ...
                            <!-- List Makanan Ditambahkan -->
                            <div v-if="foodList.length > 0" class="space-y-2 pt-2 border-t border-slate-200">
                                <p class="text-[11px] font-bold text-slate-600">Daftar Makanan Dimakan:</p>
                                <ul class="space-y-1.5 max-h-36 overflow-y-auto pr-1">
                                    <li
                                        v-for="(item, idx) in foodList"
                                        :key="idx"
                                        class="flex items-center justify-between text-xs bg-white p-2 rounded-lg border border-slate-200"
                                    >
                                        <div>
                    ...
                    <!-- Action Submit Jurnal -->
                    <div class="flex justify-end pt-4 border-t border-slate-100">
                        <button
                            type="button"
                            @click="simpanJurnal"
                            :disabled="journalForm.processing"
                            class="w-full sm:w-auto px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs rounded-xl shadow-lg shadow-emerald-600/20 transition-all disabled:opacity-50"
                        >
            ...
            <div v-if="activeTab === 'sos'" class="bg-white p-8 rounded-2xl border border-rose-200 shadow-sm text-center max-w-2xl mx-auto space-y-6">
                ...
                <div>
                    ...
                    <p class="text-xs font-semibold text-slate-500 mt-2 max-w-md mx-auto">
                        Tekan tombol di bawah untuk memanggil bantuan medis darurat RSUD Ngimbang. Lokasi dan data pasien Anda akan otomatis dikirimkan.
                    </p>
                </div>

                <button
                    @click="triggerSOS"
                    class="w-full py-5 bg-gradient-to-r from-rose-600 to-red-700 hover:from-rose-700 hover:to-red-800 text-white font-black text-base rounded-2xl shadow-xl shadow-rose-600/30 active:scale-95 transition-all tracking-wider"
                >
                    🚨 BANTU SAYA SEKARANG
                </button>
            </div>
